...
def create_tp_order():
    ...
    tp2 = float(config['tp2'])
    tp3 = float(config['tp3'])
    qty = float(config['qty'])
    if last_order['side'] == 'BUY':
        tp1_price = int(last_order_entry_price*(1+(tp1/100)))
        tp2_price = int(last_order_entry_price*(1+(tp2/100)))
        tp3_price = int(last_order_entry_price*(1+(tp3/100)))
        new_side = 'SELL'
    if last_order['side'] == 'SELL':
        ...
        tp3_price = int(last_order_entry_price*(1-(tp3/100)))
        new_side = 'BUY'
    execute_limit_order(new_side, 'BTCUSDT', round(qty/3, 3), tp1_price)
    execute_limit_order(new_side, 'BTCUSDT', round(qty/3, 3), tp2_price)
    execute_limit_order(new_side, 'BTCUSDT', round(qty/3, 3), tp3_price)
...
def execute_market_order(side, symbol, quantity):
    try:
        order = client.futures_create_order(symbol=symbol, side=side.upper(), type='MARKET', quantity=quantity)
